...
export default function Profile({ embedded = false }: { embedded?: boolean }) {
  ...
  const handleSaveLinks = () => {
    ...
    updateMutation.mutate({
      ...
      researchGate: form.researchGate || undefined,
      htwProfileUrl: form.htwProfileUrl || undefined,
      miscLink: form.miscLink || undefined,
      bookingUrl: form.bookingUrl || undefined,
    }, {
      onSuccess: () => { toast.success(p.profileSaved); setLinksEditMode(false); refetch(); },
      onError: (e) => toast.error(e.message),
    });
  };
  ...
  return (
    <div className={embedded ? "" : "min-h-screen bg-gray-50"}>
            ...
            {!editMode ? (
              <div className="flex items-center gap-2">
                <a
                  ...
                  title="Profil als PDF herunterladen"
                >
      ...
      <div className={embedded ? "space-y-6" : "max-w-3xl mx-auto px-4 py-8 space-y-6"}>
            ...
            <div className="flex items-center gap-2 pt-2 border-t border-gray-100">
              ...
              {isExaminer && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex-shrink-0 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-[#76b900] text-[#76b900] text-xs font-medium hover:bg-[#f6ffe0] transition-colors"
                >
                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="flex items-center justify-between mb-5">
            ...
            {!editMode && (
              <button
                ...
                className="flex items-center gap-1.5 text-sm font-medium px-3 py-1.5 rounded-lg border border-gray-200 hover:border-[#76b900] hover:text-[#76b900] transition-colors text-gray-600"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                {p.edit}
              </button>
        ...
        {isExaminer && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            {/* Header mit Bearbeiten/Abbrechen-Button */}
            <div className="flex items-start justify-between mb-1">
              ...
              <div className="flex items-center gap-2">
                {linksEditMode ? (
                  <>
                    <button
                      onClick={() => setLinksEditMode(false)}
                    ...
                    <button
                      ...
                      disabled={updateMutation.isPending || hasUrlErrors}
                      className="text-sm text-white px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50"
                      style={{ backgroundColor: '#76B900' }}
                      onMouseEnter={(e) => { if (!updateMutation.isPending) e.currentTarget.style.backgroundColor = '#5e9200'; }}
                      onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = '#76B900'; }}
                    >
                      {updateMutation.isPending ? (lang === 'de' ? 'Wird gespeichert…' : 'Saving…') : (lang === 'de' ? 'Speichern' : 'Save')}
                    </button>
                  </>
                ) : (
                  <button
                    onClick={() => setLinksEditMode(true)}
                    className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700 px-3 py-1.5 rounded-lg border border-gray-200 hover:border-gray-300 transition-colors"
                  >
                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {linksEditMode && hasUrlErrors && (
              <div className="mb-4 px-3 py-2 bg-red-50 border border-red-200 rounded-lg text-sm text-red-600">{p.urlSaveBlocked}</div>
            )}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              {linksEditMode ? (
                <UrlInput label={p.fieldWebsite} value={form.website} onChange={(v) => setForm((f) => ({ ...f, website: v }))} onBlur={() => autoSaveField({ website: form.website })} placeholder={p.fieldWebsitePlaceholder} errorMsg={p.urlInvalid} />
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100">
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-base font-semibold" style={{ color: "#76B900" }}>{lang === 'de' ? 'Prüfer:in in Studiengängen' : 'Study Programme Participation'}</span>
              </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <FieldView
              label={lang === "de" ? "Aktuelle Sitzung" : "Current session"}
              value={lang === "de" ? "Aktiv – geschützt" : "Active – protected"}
              notSpecified="—"
            />
          </div>
        </div>

        {user?.loginMethod === "password" && <PasswordSecurityPanel />}
