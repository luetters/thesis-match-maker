...
// ─── Icons ────────────────────────────────────────────────────────────────────
const Icons = {
  home: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a2 2 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>,
  list: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 5H7a2 2 0 002 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 001 1h2a2 2 0 001-1h2a2 2 0 012 2v12a2 2 0 01-2 2H7a2 2 0 01-2-2V7a2 2 0 012-2h2M9 5a2 2 0 002 2h2a2 2 0 002-2" /></svg>,
  ...
  users: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 11-8 0z" /></svg>,
  transfer: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 7h11m0 0l-3-3m3 3l-3 3M17 17H6m0 0l3 3m-3-3l3-3" /></svg>,
};
...
function useNavItems(pendingCount = 0, isSuperadmin = false) {
  ...
  return [
    ...
    ...(isSuperadmin ? [{ href: "/superadmin/portable-transfer", label: "Datenübernahme", icon: Icons.transfer }] : []),
          ...
          <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full mx-4 p-6">
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button
                ...
                className="px-4 py-2 rounded-xl bg-[#76B900] text-[#142800] text-sm font-semibold transition-colors hover:bg-[#8FD400] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#76B900] focus-visible:ring-offset-2 active:scale-[0.97] disabled:opacity-50"
              >
...
function Overview() {
  ...
  const confirmTwoFactor = trpc.auth.confirmTwoFactorSetup.useMutation({ onSuccess: (data) => { toast.success("Zwei-Faktor-Authentifizierung wurde aktiviert."); setRecoveryCodes(data.recoveryCodes); setTwoFactorSetup(null); setTwoFactorCode(""); refetchTwoFactor(); }, onError: (error) => toast.error(error.message) });
    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-4 lg:grid-cols-2" aria-label="Sicherheitsübersicht">
        ...
        {twoFactorStatus?.eligible && <div className="rounded-2xl border border-blue-200 bg-blue-50 p-5">
          ...
          {!twoFactorStatus.enabled && !twoFactorSetup && <button onClick={() => beginTwoFactor.mutate()} disabled={beginTwoFactor.isPending} className="mt-3 rounded-lg bg-[#76B900] px-3 py-2 text-sm font-semibold text-[#142800] transition-colors hover:bg-[#8FD400] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#76B900] focus-visible:ring-offset-2 active:scale-[0.97] disabled:opacity-50">2FA einrichten</button>}
          {twoFactorSetup && <div className="mt-3 rounded-xl bg-white p-3 text-sm text-gray-800"><img src={twoFactorSetup.qrCodeDataUrl} alt="QR-Code für 2FAS" className="h-36 w-36 rounded" /><p className="mt-2 break-all text-xs">Manueller Schlüssel: {twoFactorSetup.manualKey}</p><input value={twoFactorCode} onChange={(event) => setTwoFactorCode(event.target.value.replace(/\D/g, "").slice(0, 6))} placeholder="Sechsstelliger Code" inputMode="numeric" className="mt-3 w-full rounded border p-2" /><button onClick={() => confirmTwoFactor.mutate({ code: twoFactorCode })} disabled={twoFactorCode.length !== 6 || confirmTwoFactor.isPending} className="mt-2 rounded-lg bg-[#76B900] px-3 py-2 text-sm font-semibold text-[#142800] transition-colors hover:bg-[#8FD400] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#76B900] focus-visible:ring-offset-2 active:scale-[0.97] disabled:opacity-50">Aktivierung bestätigen</button></div>}
