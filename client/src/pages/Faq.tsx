...
import { GUIDE_PDF_URLS, GUIDE_PRINT_PDF_URLS } from "@/lib/guideAssets";
import { getGuideDownloadSessionKey, GUIDE_DOWNLOAD_KEYS } from "@shared/guideAssets";
import { ArrowLeft, CheckCircle2, ChevronDown, Download, FileQuestion, HeartHandshake, ListChecks, MessageSquarePlus, Printer, Search, Send, ShieldCheck, Sparkles, ThumbsDown, ThumbsUp } from "lucide-react";
...
export const FAQ_GUIDE_PRINT_LINKS = [GUIDE_PRINT_PDF_URLS.firstExaminer, GUIDE_PRINT_PDF_URLS.secondExaminer, GUIDE_PRINT_PDF_URLS.administration] as const;
        ...
        <section className="container py-10 sm:py-14">
          <section className="mb-10 rounded-3xl border border-[#cdeca3] bg-gradient-to-r from-[#f4fbe9] to-white p-6 shadow-sm sm:p-8" aria-labelledby="guide-download-title">
            ...
            <div className="mt-6 grid gap-3 lg:grid-cols-3">
              {copy.guides.map((guide, index) => <article key={guide.title} className="flex min-h-32 flex-col justify-between rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
                ...
                <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-sm font-semibold">
                  <a href={guideLinks[index]} download onClick={() => handleGuideDownload(FAQ_GUIDE_DOWNLOAD_KEYS[index])} className="inline-flex items-center gap-2 text-[#527b00] transition hover:text-[#365800] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#76B900]"><Download className="h-4 w-4" />{copy.guideDownload}</a>
                  <a href={guidePrintLinks[index]} download onClick={() => handleGuideDownload(FAQ_GUIDE_DOWNLOAD_KEYS[index])} className="inline-flex items-center gap-2 text-slate-700 transition hover:text-slate-950 focus:outline-none focus-visible:ring-2 focus-visible:ring-slate-600"><Printer className="h-4 w-4" />{copy.guidePrintDownload}</a>
                </div>
              </article>)}
            </div>
          </section>
